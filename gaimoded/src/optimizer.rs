...
impl OptimizerObject for ProcessOptimizer {
    ...
    fn unoptimize(&mut self) -> anyhow::Result<()> {
        let mut failed = false;
        if let Err(why) = self.rollback_nc_ionc() {
            tracing::error!("Failed to reset niceness and ioniceness: {}", why);
            failed = true;
        }
        ...
        if self.settings.cpu_affinity.enabled {
            ...
            for task in tasks {
                ...
                {
                    tracing::error!("Setting aff. mask failure: {}", why);
                    failed = true;
                }
            }
        }
        if failed {
            return Err(anyhow::anyhow!(
                "One or more operations failed when performing a reset"
            ));
        }
        Ok(())
    }
